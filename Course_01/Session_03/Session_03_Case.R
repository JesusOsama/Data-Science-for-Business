...
# 1. Import the data
medals <- read_csv("Course_01/Session_01/medals_total.csv")
spec(medals)
...
medals_long <- medals %>%
  ...
  pivot_longer(cols = c("Gold Medal", "Silver Medal", "Bronze Medal"), 
               names_to = "Medal_Type", 
               values_to = "Count")
...
ggplot(medals_long %>% 
  ...
  coord_flip() +
  scale_fill_manual(values = c( "brown","gold", "grey")) +
...
ggplot(medals, aes(x = "Gold Medal", y = Total)) +
...
country_of_interest <- "Japan"  # Change this to any country in your dataset

country_medals <- medals %>%
  filter(country == country_of_interest) %>%
  select("Gold Medal", "Silver Medal", "Bronze Medal") %>%
...
correlation_matrix <- cor(medals[, c("Gold Medal", "Silver Medal", "Bronze Medal", "Total")])
...
plot_medal_ratio(medals, "Gold Medal")
